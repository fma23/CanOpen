 ...
 * @file        CO_PDO.c
 * @ingroup     CO_PDO
 * @version     SVN: \$Id: CO_PDO.c 46 2013-08-24 09:18:16Z jani22 $
...
/* Private variable ----------------------------------------------------------*/
/* Private function ----------------------------------------------------------*/
static int16_t CO_TPDOsend(CO_TPDO_t *TPDO);
static void CO_PDO_receive(void *object, CO_CANrxMsg_t *msg);
...
static CO_SDO_abortCode_t CO_ODF_RPDOmap(CO_ODF_arg_t *ODF_arg)
{
    ...
    if(ODF_arg->subIndex == 0)
    {
        ...
        /* configure mapping */
        return CO_RPDOconfigMap(RPDO, *value);    // VJ to remove warnings
    }
    ...
    else
    {
        ...
        return CO_PDOfindMap(    // VJ to remove warnings
                RPDO->SDO,
               *value,
                0,
               &pData,
...
static CO_SDO_abortCode_t CO_ODF_TPDOmap(CO_ODF_arg_t *ODF_arg)
{
    ...
    if(ODF_arg->subIndex == 0)
    {
        ...
        /* configure mapping */
        return CO_TPDOconfigMap(TPDO, *value);    // VJ to remove warnings
    }
